class Solution:
    def robotSim(self, commands, obstacles):
        ...
        result = []
        x, y = 0, 0
        dimension = ['north', 'east', 'south', 'west']
        current_index = 0
        for i in range(0, len(commands)):
            # print('-------')
            # print(commands[i])
            # print(dimension[current_index])
            if commands[i] == -2:
                current_index -= 1
                if current_index == -1:
                    current_index = 3
            ...
            if dimension[current_index] == 'north':
                y += commands[i]
                ob = self.getObstacleOnTheYWay(obstacles, x, y-commands[i], y, True)
                print(ob)
                if len(ob) != 0:
                    y = ob[1] - 1
            elif dimension[current_index] == 'east':
                x += commands[i]
                ob = self.getObstacleOnTheXWay(obstacles, y, x-commands[i], x, True)
                print(ob)
                if len(ob) != 0:
                    x = ob[0] - 1
            elif dimension[current_index] == 'south':
                y -= commands[i]
                ob = self.getObstacleOnTheYWay(obstacles, x, y+commands[i], y, False)
                print(ob)
                if len(ob) != 0:
                    y = ob[1] + 1
            else:
                x -= commands[i]
                ob = self.getObstacleOnTheXWay(obstacles, y, x+commands[i], x, False)
                print(ob)
                if len(ob) != 0:
                    x = ob[0] + 1
            print(x, y)
    ...
    def getObstacleOnTheXWay(self, obstacles, y, oldX, newX, postive):
        result = []
        for i in range(0, len(obstacles)):
            # if obstacles[i][1] == y:
            #     print(obstacles[i])
            if obstacles[i][1] == y and (oldX < obstacles[i][0] <= newX or newX <= obstacles[i][0] < oldX):
    ...
    def getObstacleOnTheYWay(self, obstacles, x, oldY, newY, postive):
        result = []
        for i in range(0, len(obstacles)):
            # if obstacles[i][0] == x:
            #     print(obstacles[i])
            if obstacles[i][0] == x and (oldY < obstacles[i][1] <= newY or newY <= obstacles[i][1] < oldY):
                result.append(obstacles[i])
        if len(result) > 0:
